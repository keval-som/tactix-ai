...
export const DebriefStage = () => {
  const { actionCards, postCallReport, reset } = useNegotiationStore();

  // Prefer real report; fall back to computed values from action cards
  const totalSavings = postCallReport?.stats.savings
    ?? actionCards.reduce((sum, c) => sum + (c.savings || 0), 0);
  const trapsDeflected = postCallReport?.stats.traps_deflected
    ?? actionCards.filter(c => c.type === 'PSYCH_CUE' || c.type === 'VISION_GOTCHA').length;
  const outcomeLabel = postCallReport?.stats.outcome_sentiment ?? 'Strong Position';

  const sortedCards = [...actionCards].sort((a, b) => a.timestamp - b.timestamp);

  const draftEmail = postCallReport?.follow_up_email ?? `Subject: Follow-up on Our Discussion
...
[Your Name]`;
  ...
  const stats = [
    ...
    { label: 'Outcome Sentiment', value: outcomeLabel, icon: Brain, color: 'text-primary' },
  ];
      ...
      <div className="max-w-4xl mx-auto p-6 space-y-8">
        ...
        >
          ...
          <p className="text-sm text-muted-foreground">
            {postCallReport ? 'Generated by TactiX AI' : 'Your negotiation intelligence report'}
          </p>
        </motion.div>
        ...
        {sortedCards.length > 0 && (
          <div>
            <h3 className="text-xs font-mono font-bold text-muted-foreground tracking-wider uppercase mb-4">
              Memory Timeline ({sortedCards.length} alerts)
            </h3>
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex items-center gap-1 px-2 py-1 rounded-lg bg-secondary text-secondary-foreground text-xs font-mono hover:bg-muted transition-colors"
              >
                <Copy size={12} /> Copy
              </button>
              <button
                onClick={() => {
                  const subject = encodeURIComponent('Follow-up on Our Discussion');
                  const body = encodeURIComponent(draftEmail);
                  window.open(`mailto:?subject=${subject}&body=${body}`);
                }}
                className="flex items-center gap-1 px-2 py-1 rounded-lg bg-primary text-primary-foreground text-xs font-mono hover:bg-primary/90 transition-colors"
              >
                <Mail size={12} /> Send
              </button>
            </div>
